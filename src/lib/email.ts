...
function orderItemsHtml(order: Order) {
  return order.items.map(item => `
    <div style="${styles.row}">
      <span style="color:#555">${escapeHtml(item.product_title)} × ${item.quantity}</span>
    ...
    <div style="${styles.body}">
      <div style="${styles.container}">
        ...
        <div style="${styles.body_inner}">
          ...
          <p style="${styles.p}">Hola ${escapeHtml(order.customer.name)}, recibimos tu pedido y ya estamos gestionando tu compra.</p>

          <div style="${styles.orderBox}">
            <p style="font-size:11px;font-weight:700;color:#999;text-transform:uppercase;letter-spacing:.06em;margin:0 0 12px">Pedido ${escapeHtml(order.order_number)}</p>
    ...
    <div style="${styles.body}">
      <div style="${styles.container}">
        ...
        <div style="${styles.body_inner}">
          ...
          <p style="${styles.p}">Hola ${escapeHtml(order.customer.name)}, tu pedido <strong>${escapeHtml(order.order_number)}</strong> fue despachado.</p>
          ...
          <div style="${styles.orderBox}">
            ...
            <p style="font-size:22px;font-weight:700;color:#111;margin:0;letter-spacing:.05em">${escapeHtml(order.tracking_number)}</p>
            <p style="font-size:12px;color:#999;margin:4px 0 0">Transportista: ${escapeHtml(order.tracking_carrier || 'USPS')}</p>
          </div>
    ...
    <div style="${styles.body}">
      <div style="${styles.container}">
        ...
        <div style="${styles.body_inner}">
          ...
          <p style="${styles.p}">Hola ${escapeHtml(order.customer.name)}, el producto de tu preventa ya está disponible y listo para despachar.</p>
    ...
    html: `
      <div style="${styles.body}">
        <div style="${styles.container}">
          ...
          <div style="${styles.body_inner}">
            <p style="${styles.p}"><strong>Nombre:</strong> ${escapeHtml(name)}</p>
            <p style="${styles.p}"><strong>Email:</strong> ${escapeHtml(email)}</p>
            <p style="${styles.p}"><strong>Mensaje:</strong></p>
            <div style="background:#F7F7F7;border-radius:10px;padding:16px;font-size:14px;color:#333;line-height:1.6">${escapeHtml(message)}</div>
          </div>
        </div>
      </div>`,
  });
