...
export async function ensureInit() {
  ...
  const statements = [
    ...
    `ALTER TABLE cb_galleries ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT true`,
    `ALTER TABLE cb_galleries ADD COLUMN IF NOT EXISTS sort_order INTEGER NOT NULL DEFAULT 9999`,
    `ALTER TABLE cb_galleries ADD COLUMN IF NOT EXISTS seo_description TEXT DEFAULT ''`,
    `ALTER TABLE cb_galleries ADD COLUMN IF NOT EXISTS source_type TEXT NOT NULL DEFAULT 'coverbrowser'`,
    `ALTER TABLE cb_galleries ADD COLUMN IF NOT EXISTS custom_covers JSONB DEFAULT '[]'`,
    // ── COVERBROWSER COVERS ────────────────────────────────────────────────
