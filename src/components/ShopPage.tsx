...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/products?status=published&limit=200')
      .then(r => r.json()).then(d => {
        const items = (d.data?.items || []).map(mapProduct);
        setCatalog(items);
        // Validate recently viewed against live catalog:
        // - removes deleted products
        // - refreshes prices and slugs from live data
        try {
          const rvIds: string[] = JSON.parse(localStorage.getItem('ltc_recently_viewed') || '[]')
            .map((r: any) => r.id || r).filter(Boolean);
          const catalogMap = new Map(items.map((p: Product) => [p.id, p]));
          const fresh = rvIds
            .map((id: string) => catalogMap.get(id))
            .filter(Boolean) as Product[];
          setRecentlyViewed(fresh.slice(0, 6));
        } catch {}
      })
      .catch(() => {});
  }, []);
  ...
  function addToRecentlyViewed(p: Product) {
    try {
      // Store only IDs — prices/data refreshed from live API on load
      const current: string[] = JSON.parse(localStorage.getItem('ltc_recently_viewed') || '[]')
        .map((r: any) => r.id || r).filter(Boolean);
      const filtered = current.filter((id: string) => id !== p.id);
      const updated = [p.id, ...filtered].slice(0, 6);
      localStorage.setItem('ltc_recently_viewed', JSON.stringify(updated));
      // Update display state with fresh product object
      setRecentlyViewed(prev => {
        const withoutThis = prev.filter(r => r.id !== p.id);
        return [p, ...withoutThis].slice(0, 6);
      });
    } catch {}
  }
      ...
      {isAmazon ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {p.slug && (
              <a href={`/producto/${p.slug}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#CC0000', color: 'white', borderRadius: 10, padding: '0 16px', textDecoration: 'none', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap', height: 44, flexShrink: affiliateUrl ? 0 : undefined, flex: affiliateUrl ? undefined : 1 }}>
                Ver producto →
              </a>
            )}
          </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 14 }}>
            ...
            {p.slug && (
              <a href={`/producto/${p.slug}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', padding: 10, background: '#f5f5f5', border: '1px solid #e0e0e0', color: '#333', fontSize: 13, fontWeight: 600, borderRadius: 10, textDecoration: 'none' }}>
