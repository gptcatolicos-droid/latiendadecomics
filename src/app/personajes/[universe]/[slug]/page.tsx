...
export async function generateMetadata({
  ...
  params: { universe: string; slug: string };
}): Promise<Metadata> {
  ...
  return {
    title,
    description: desc,
    keywords: [char.name.toLowerCase(), char.realName.toLowerCase(), `${char.name} comics`, `${char.name} poderes`, uniLabel.toLowerCase(), 'personajes comics'],
    openGraph: {
      title,
      description: desc,
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#0D0D0D', color: '#fff' }}>
        ...
        <nav style={{ background: '#111', borderBottom: '1px solid #222', position: 'sticky', top: 0, zIndex: 50 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px', height: 52, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none', flexShrink: 0 }}>
              <div style={{ width: 26, height: 26, background: '#CC0000', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 11 }}>📚</span>
              </div>
              <span style={{ fontFamily: 'var(--font-oswald)', fontSize: 13, color: '#fff', letterSpacing: 1 }}>
                La Tienda de <span style={{ color: '#CC0000' }}>Comics</span>
              </span>
            </Link>
            <span style={{ color: 'rgba(255,255,255,.3)', fontSize: 11 }}>›</span>
            <Link href="/personajes" style={{ color: 'rgba(255,255,255,.5)', fontSize: 11, textDecoration: 'none' }}>Personajes</Link>
            <span style={{ color: 'rgba(255,255,255,.3)', fontSize: 11 }}>›</span>
            <span style={{ background: uniBg, color: '#fff', fontSize: 10, padding: '2px 6px', borderRadius: 4 }}>{uniLabel}</span>
            <span style={{ color: 'rgba(255,255,255,.3)', fontSize: 11 }}>›</span>
            <span style={{ color: '#fff', fontSize: 11 }}>{char.name}</span>
        ...
        {/* ── CHARACTER HERO ── */}
        <div style={{ background: 'linear-gradient(180deg, #161616 0%, #0D0D0D 100%)', borderBottom: '1px solid #222', padding: '32px 16px' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 28, alignItems: 'start' }}>
            {/* Image */}
            <div style={{ flexShrink: 0 }}>
              <img
                ...
                style={{ width: 'clamp(140px, 25vw, 200px)', height: 'auto', aspectRatio: '3/4', objectFit: 'cover', objectPosition: 'top', borderRadius: 12, border: `2px solid ${uniBg}`, display: 'block', background: '#222' }}
              />
            </div>
            ...
            <div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                <span style={{ background: uniBg, color: '#fff', fontSize: 11, padding: '3px 10px', borderRadius: 12, fontWeight: 700 }}>
                  {uniLabel}
                </span>
                <span style={{ background: alignColor + '33', color: alignColor, fontSize: 11, padding: '3px 10px', borderRadius: 12, fontWeight: 600, border: `1px solid ${alignColor}44` }}>
                  {alignLabel}
                </span>
              </div>

              <h1 style={{ fontFamily: 'var(--font-oswald), serif', fontSize: 'clamp(24px, 5vw, 44px)', fontWeight: 700, color: '#fff', margin: '0 0 4px', letterSpacing: 1 }}>
                {char.name}
              </h1>
              {char.realName !== char.name && (
                <p style={{ color: 'rgba(255,255,255,.45)', fontSize: 14, margin: '0 0 16px' }}>
                  Identidad: <b style={{ color: 'rgba(255,255,255,.7)' }}>{char.realName}</b>
                </p>
              )}

              <p style={{ color: 'rgba(255,255,255,.7)', fontSize: 14, lineHeight: 1.7, margin: '0 0 20px', maxWidth: 600 }}>
                {char.description}
              </p>
              ...
              {char.teams.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {char.teams.map(t => (
                      <span key={t} style={{ background: '#1a1a1a', border: '1px solid rgba(255,255,255,.12)', color: 'rgba(255,255,255,.65)', fontSize: 11, padding: '3px 10px', borderRadius: 12 }}>
                        {t}
                      </span>
                    ))}
                  </div>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 300px', gap: 24, alignItems: 'start' }}>

          {/* ── LEFT: Powers + Galleries ── */}
          <div>
            {/* Powers */}
            <section style={{ marginBottom: 28 }}>
              <h2 style={{ fontFamily: 'var(--font-oswald)', fontSize: 18, color: '#fff', margin: '0 0 14px', letterSpacing: 1 }}>
            ...
            {char.relatedGalleries.length > 0 && (
              <section style={{ marginBottom: 28 }}>
                <h2 style={{ fontFamily: 'var(--font-oswald)', fontSize: 18, color: '#fff', margin: '0 0 14px', letterSpacing: 1 }}>
                  📚 Galerías de Portadas
                </h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10 }}>
                  {char.relatedGalleries.map(gallerySlug => (
                    <Link key={gallerySlug} href={`/blog/covers/${gallerySlug}`} style={{ textDecoration: 'none', display: 'block' }}>
                      <div style={{ borderRadius: 8, overflow: 'hidden', border: '1px solid rgba(255,255,255,.1)', background: '#161616' }}
                        onMouseEnter={e => (e.currentTarget.style.borderColor = '#CC0000')}
                        onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,.1)')}
                      >
                        <img
                          src={`https://www.coverbrowser.com/image/${gallerySlug}/1-1.jpg`}
                          alt={gallerySlug.replace(/-/g, ' ')}
                          loading="lazy"
                          referrerPolicy="no-referrer"
                          style={{ width: '100%', aspectRatio: '2/3', objectFit: 'cover', display: 'block', background: '#222' }}
                        />
                        <div style={{ padding: '6px 8px', fontSize: 11, color: 'rgba(255,255,255,.6)', textTransform: 'capitalize' }}>
                          {gallerySlug.replace(/-/g, ' ')}
                        </div>
                      </div>
                    </Link>
                  ))}
        ...
        {related.length > 0 && (
          <div style={{ borderTop: '1px solid #222', padding: '28px 16px' }}>
            <div style={{ maxWidth: 1000, margin: '0 auto' }}>
              <h2 style={{ fontFamily: 'var(--font-oswald)', fontSize: 18, color: '#fff', margin: '0 0 16px', letterSpacing: 1 }}>
                Otros Personajes de {uniLabel}
              </h2>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10 }}>
                {related.map(r => (
                  <Link key={r.slug} href={`/personajes/${r.universe}/${r.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
                    <div style={{ borderRadius: 8, overflow: 'hidden', border: '1px solid rgba(255,255,255,.08)', background: '#161616', cursor: 'pointer' }}
                      onMouseEnter={e => (e.currentTarget.style.borderColor = '#CC0000')}
        ...
        <div style={{ position: 'sticky', bottom: 0, background: 'rgba(13,13,13,.97)', backdropFilter: 'blur(12px)', borderTop: '1px solid rgba(255,255,255,.1)', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,.5)', flex: 1, minWidth: 140 }}>
            <b style={{ color: '#fff' }}>🤖 Jarvis:</b> ¿Quieres comprar cómics de {char.name}?
          </span>
          <Link href="/personajes" style={{ border: '1px solid rgba(255,255,255,.15)', color: 'rgba(255,255,255,.55)', borderRadius: 6, padding: '5px 12px', fontSize: 12, textDecoration: 'none' }}>
            ← Todos los personajes
          </Link>
          <Link href="/catalogo" style={{ background: '#CC0000', color: '#fff', borderRadius: 6, padding: '5px 12px', fontSize: 12, textDecoration: 'none', fontWeight: 600 }}>
            Ver Catálogo
          </Link>
