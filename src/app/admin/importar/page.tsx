...
export default function BulkImportPage() {
  ...
  async function resetProducts() {
    if (!confirm('¿Borrar TODOS los productos, pedidos y cupones? Esta acción no se puede deshacer.')) return;
    setResetting(true);
    const res = await fetch('/api/admin/reset-db', { method: 'POST' });
    const data = await res.json();
    setResetting(false);
    setResetDone(data.success);
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 820 }}>
      ...
      <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '14px 18px', marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#dc2626', marginBottom: 2 }}>Limpiar base de datos</div>
          <div style={{ fontSize: 12, color: '#ef4444' }}>Borra productos, pedidos y cupones. Preserva admin y config.</div>
          {resetDone && <div style={{ fontSize: 12, color: '#15803d', marginTop: 3, fontWeight: 600 }}>✓ BD limpiada correctamente</div>}
        </div>
