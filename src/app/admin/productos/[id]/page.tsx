    ...
    <div style={{ padding: '28px 32px', maxWidth: 780 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #ebebeb', borderRadius: 14, padding: 20 }}>
          {/* SEO Score header */}
          {(() => {
            const score = calculateProductSeoScore({ ...form, images });
            const color = getSeoColor(score.grade);
            return (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                <h2 style={{ fontSize: 14, fontWeight: 700, color: '#111', flex: 1 }}>SEO</h2>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {/* Score ring */}
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: `${color}18`, border: `3px solid ${color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color, lineHeight: 1 }}>{score.total}</span>
                    <span style={{ fontSize: 8, color, fontWeight: 700 }}>{score.grade}</span>
                  </div>
                  <div style={{ fontSize: 11, color: '#6b7280' }}>
                    <div style={{ fontWeight: 600, color }}>Score SEO: {score.grade}</div>
                    <div>{score.items.filter(i => i.passed).length}/{score.items.length} checks</div>
                  </div>
                </div>
              </div>
            );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {(() => {
              ...
              if (!failed.length) return <div style={{ fontSize: 12, color: '#16a34a', fontWeight: 600 }}>✅ Todos los checks SEO pasados</div>;
              return (
                <div style={{ background: '#fafafa', borderRadius: 8, padding: '10px 12px', border: '1px solid #f0f0f0' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#374151', marginBottom: 8 }}>⚠️ Mejoras SEO pendientes</div>
                  {failed.map((item, i) => (
                    <div key={i} style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, display: 'flex', gap: 6 }}>
                      <span style={{ color: '#f59e0b' }}>○</span>
                      <span><b>{item.label}</b>{item.tip ? ` — ${item.tip}` : ''}</span>
                    </div>
                  ))}
                </div>
              );
